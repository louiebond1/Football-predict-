...
function money(pence = 0) { const n = Number(pence || 0) / 100; return `£${Number.isInteger(n) ? n.toFixed(0) : n.toFixed(2)}`; }
...
function addStakesCard(group) {
  ...
  if (!overview) return;
  overview.querySelector('.kp-stakes-card')?.remove();
...
function applyFunPresentation(group) {
  ...
  [...document.querySelectorAll('.kp3-group-menu .kp3-nav-row')].forEach(row => {
    if (/^Payments$/i.test(row.querySelector('.kp3-nav-copy strong')?.textContent?.trim() || '')) row.classList.toggle('kp-mode-hidden', fun);
  });
...
function showAdminPage(adminView, target) {
  [...adminView.querySelectorAll(':scope > .kp-admin-page')].forEach(page => { page.hidden = page !== target; });
  window.scrollTo({ top: 0, behavior: 'auto' });
}

async function enhanceAdminPlayMode() {
  const menu = document.querySelector('.kp-admin-menu');
  const menuPage = menu?.closest('.kp-admin-page');
  const adminView = menuPage?.closest('.kp-admin-view');
  if (!menu || !menuPage || !adminView || menu.querySelector('.kp-playmode-nav')) return;
  ...
  page.className = 'kp-admin-page kp-playmode-page';
  page.hidden = true;
  page.innerHTML = `<section class="kp3-drill-header kp-admin-header"><button type="button" class="kp3-back" aria-label="Back"><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg></button><div><h1>Play mode</h1><p>Choose money mode or play for fun</p></div></section>
    <section class="kp-admin-section kp-mode-section"><div class="kp-mode-form">
      <label class="kp-mode-toggle"><span class="kp-mode-toggle-copy"><strong>Require weekly payment</strong><small>Turn this off and everyone can predict without paying.</small></span><span class="kp-switch"><input type="checkbox" class="kp-payment-required" ${group.payments_required !== false ? 'checked' : ''}><span></span></span></label>
      <label>Winner prize / reward<textarea class="kp-winner-prize" maxlength="240" placeholder="Optional — e.g. winner chooses the pub">${esc(group.winner_prize || '')}</textarea></label>
      <label>Loser punishment / forfeit<textarea class="kp-loser-punishment" maxlength="240" placeholder="Optional — e.g. loser wears a rival shirt">${esc(group.loser_punishment || '')}</textarea></label>
      <button type="button" class="kp-mode-save">Save play mode</button><small class="kp-mode-status"></small>
    </div></section>`;
  adminView.append(page);

  const modeNav = document.createElement('button');
  modeNav.type = 'button';
  modeNav.className = 'kp3-nav-row kp-admin-nav-row kp-playmode-nav';
  const modeMeta = group.payments_required === false ? 'For fun' : `${money(group.stake_pence)} / week`;
  modeNav.innerHTML = `<span class="kp3-nav-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 4h10v3a5 5 0 0 1-10 0V4z"/><path d="M7 5.5H4.5A2.5 2.5 0 0 0 7 8M17 5.5h2.5A2.5 2.5 0 0 1 17 8"/><path d="M12 12v4M8.5 20h7"/></svg></span><span class="kp3-nav-copy"><strong>Play mode</strong><small>Money, rewards & forfeits</small></span><span class="kp3-nav-meta">${esc(modeMeta)}</span><span class="kp3-nav-chevron"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6l6 6-6 6"/></svg></span>`;

  const groupRow = [...menu.querySelectorAll('.kp-admin-nav-row')].find(row => /^Group & invite$/i.test(row.querySelector('.kp3-nav-copy strong')?.textContent?.trim() || ''));
  if (groupRow) menu.insertBefore(modeNav, groupRow); else menu.append(modeNav);

  modeNav.addEventListener('click', () => showAdminPage(adminView, page));
  page.querySelector('.kp3-back').addEventListener('click', () => showAdminPage(adminView, menuPage));
  page.querySelector('.kp-mode-save').addEventListener('click', async () => {
    const status = page.querySelector('.kp-mode-status');
    const button = page.querySelector('.kp-mode-save');
    const paymentsRequired = page.querySelector('.kp-payment-required').checked;
    ...
    group.payments_required = paymentsRequired;
    group.winner_prize = winnerPrize;
    group.loser_punishment = loserPunishment;
    modeNav.querySelector('.kp3-nav-meta').textContent = paymentsRequired ? `${money(group.stake_pence)} / week` : 'For fun';
    status.textContent = 'Saved ✓';
    invalidateContext();
