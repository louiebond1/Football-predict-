(() => {
  ...
  document.querySelectorAll('.bottom-nav .nav-item').forEach(btn=>btn.addEventListener('click',()=>{if(btn.dataset.tab==='gw'){render();if(!S.loaded)load().then(render)}else document.body.classList.remove('kp-native-matchday')}));
