(() => {
  ...
  async function load(){if(S.loading)return;S.loading=true;try{
    ...
    const {data:groups}=await client.from('groups').select('*').order('created_at');
    const stored=(()=>{try{return sessionStorage.getItem('kp-active-group-v1')||localStorage.getItem('kp-active-group-v1')||''}catch{return''}})();
    S.groupId=(groups||[]).find(g=>g.id===stored)?.id||(groups||[])[0]?.id||null; if(!S.groupId)return;
    const {data:gw}=await client.rpc('ensure_current_gameweek',{p_group_id:S.groupId}); S.gameweekId=gw;
    const uid=session.user.id;
    const [{data:payments},{data:preds},fx]=await Promise.all([
      client.from('payments').select('*').eq('group_id',S.groupId).eq('gameweek_id',S.gameweekId).eq('user_id',uid),
      client.from('predictions').select('*').eq('group_id',S.groupId).eq('gameweek_id',S.gameweekId).eq('user_id',uid),
    ...
    S.loaded=true;
  }catch(e){console.error('KickPot Matchday load',e)}finally{S.loading=false}}
  ...
  function render(){if(!isGW())return;document.body.classList.add('kp-native-matchday');styleBrand();const screen=$('#screen');if(!screen)return;if(!S.loaded){screen.innerHTML='<div class="kp-native-loading">Loading fixtures…</div>';return;}const rn=(String(S.round).match(/\d+/)||['4'])[0],next=S.fixtures.filter(f=>new Date(f.kickoff)>new Date()).sort((a,b)=>new Date(a.kickoff)-new Date(b.kickoff))[0],first=S.fixtures.slice().sort((a,b)=>new Date(a.kickoff)-new Date(b.kickoff))[0];screen.innerHTML=`<div class="kp-native-tabs"><span>Fixtures</span></div><section class="kp-native-hero"><div class="kp-native-eyebrow">Matchday ${rn}</div><h1>Premier<br>League</h1><div class="kp-native-count"><small>Picks close in</small><strong>${esc(next?heroRel(next.kickoff):'Closed')}</strong></div></section><section class="kp-native-list"><div class="kp-native-date">${esc(first?fmtDate(first.kickoff):'')}</div>${S.fixtures.map(fixtureHTML).join('')}<button class="kp-native-lock-button" id="kpNativeLock" ${!S.paid?'disabled':''}>${S.paid?'Lock in my picks':'Predictions unavailable'}</button><div class="kp-native-status" id="kpNativeStatus"></div></section>`}
  async function save(){const client=sb(),status=$('#kpNativeStatus'),btn=$('#kpNativeLock');if(!client||!S.paid)return;const {data:{session}}=await client.auth.getSession();if(!session)return;const open=S.fixtures.filter(f=>Date.now()<new Date(f.kickoff).getTime());const rows=open.map(f=>{const p=draftFor(String(f.id));return{group_id:S.groupId,gameweek_id:S.gameweekId,fixture_id:f.id,user_id:session.user.id,predicted_home:p.home,predicted_away:p.away}});if(!rows.length){status.textContent='No open fixtures left to predict.';return}btn.disabled=true;btn.textContent='Saving…';const {error}=await client.from('predictions').upsert(rows,{onConflict:'group_id,fixture_id,user_id'});if(error){status.textContent=error.message;btn.disabled=false;btn.textContent='Lock in my picks';return}status.textContent='✓ Picks locked in';const {data:preds}=await client.from('predictions').select('*').eq('group_id',S.groupId).eq('gameweek_id',S.gameweekId).eq('user_id',session.user.id);S.predictions=Object.fromEntries((preds||[]).map(p=>[String(p.fixture_id),p]));btn.textContent='Locked in';setTimeout(()=>{btn.disabled=false;btn.textContent='Lock in my picks'},900)}
  ...
  document.querySelectorAll('.bottom-nav .nav-item').forEach(btn=>btn.addEventListener('click',()=>{setTimeout(async()=>{if(btn.dataset.tab==='gw'){await load();render()}else document.body.classList.remove('kp-native-matchday')},0)}));
  const mo=new MutationObserver(()=>{if(isGW()&&!$('#screen .kp-native-hero'))setTimeout(render,0)});const screen=$('#screen');if(screen)mo.observe(screen,{childList:true});
  (async()=>{if(isGW()){await load();render()}})();
})();