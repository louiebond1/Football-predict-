(() => {
  ...
  // Safety net for a rare tab-entry race (same fix applied to the Live tab's
  // equivalent file, live-reference-table.js - see the comment there for the
  // full reasoning): give the Matchday tab a couple more chances, shortly
  // after entry, to notice it's active but not actually showing its own
  // markup, and redraw itself. render() is a cheap no-op when everything's
  // already correct.
  function ensureGWRendered(){if(isGW()&&S.loaded&&!$('#screen .kp-native-hero'))render()}
  document.querySelectorAll('.bottom-nav .nav-item').forEach(btn=>btn.addEventListener('click',()=>{if(btn.dataset.tab==='gw'){render();if(!S.loaded)load().then(render);setTimeout(ensureGWRendered,400);setTimeout(ensureGWRendered,1200)}else document.body.classList.remove('kp-native-matchday')}));
